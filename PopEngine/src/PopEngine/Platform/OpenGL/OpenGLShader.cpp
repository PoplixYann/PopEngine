...
namespace PE
{
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			PE_CORE_ASSERT(false, "Could not open file '{0}'", filepath);
		}
		return result;
	}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			PE_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specifie");
	...
	{
		PE_CORE_ASSERT(shaderSources.size() <= 2, "We only support 2 shaders max for now");
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				PE_LOG_CORE_ERROR("{0}", infoLog.data());
